...
for i=1:m
    Xit = X(i,:);
    a1 = Xit;
    z2 = sigmoid(Xit*Theta1');
    a2 = [1 z2];
    z3 = a2*Theta2';
    hit = sigmoid(z3);
    
    yit = zeros(1,K); %y iteration 
    yit(y(i)) = 1; % if y(i) is 1-10 then yit = 1, else 0;
    ystore(i,:) = yit;
    J = J + sum(-yit.*log(hit) - (1-yit).* log(1-hit));
end
J = J/m;
...
del1_store = zeros(size(Theta1));
del2_store = zeros(size(Theta2));
...
for t = 1:m
    ...
    del1_store = del1_store + del2(2:end)'*a1;
    del2_store = del2_store + del3'*a2;
    
end

%unregularized gradient
Theta1_grad = del1_store/m;
Theta2_grad = del2_store/m;
